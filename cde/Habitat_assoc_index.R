...
fish_overlap$quantile_2 <- ntile(fish_overlap$PC1,4)   
...
# Distribution of all data points along the PC1 gradient
...
p1_colour <-  fish_overlap %>%
  mutate(species_n = fct_reorder(species_n, PC1,.desc=TRUE)) %>% 
  ggplot(aes(x = PC1, y = species_n, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 2, rel_min_height = 0.01) +
  xlab("") +
  ylab("") +
  xlim(-4, 6) +
  scale_fill_gradientn(colors = c("steelblue4",
                                  "steelblue", 
                                  "seagreen3",  
                                  "seagreen4")) + 
  theme_bw() + 
  geom_vline(xintercept = -0.80788161, linetype  = "dashed", colour ="888", linewidth = 0.5) +
  geom_vline(xintercept = -0.04131737 , linetype  = "dashed", colour = "888", linewidth = 0.5) +
  geom_vline(xintercept = 1.19393367, linetype  = "dashed", colour = "888", linewidth = 0.5) +
  theme(legend.position = "none") +
...
plot_a <- plot_grid(hab_1,hab_2,hab_3,hab_4,ncol=1, labels = "a") 

plot_b <- plot_grid(p1_colour, p2,ncol=1, rel_widths = c(1,1.5), rel_heights = c(6,.5), align = "v",labels="b")
